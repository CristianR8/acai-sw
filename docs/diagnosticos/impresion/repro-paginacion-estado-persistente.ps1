...
$document.add_PrintPage({ param($sender, $event)
  ...
  while ($state.LineIndex -lt $renderLines.Count -and ($y + $lineHeight) -le ($pageHeight - 16)) {
    $lineText = [string]$renderLines[$state.LineIndex]
  ...
  $state.PageIndex++
  $event.HasMorePages = $state.LineIndex -lt $renderLines.Count
})
